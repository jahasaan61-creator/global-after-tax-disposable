...
export const queryGemini = async (prompt: string, country: string): Promise<string> => {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      2. DO NOT hallucinate numbers. If uncertain, state that you cannot verify.
      3. Answer concisely with verifiable numeric data.
      4. The response MUST be grounded in search results.
      `,
      config: {
        tools: [{ googleSearch: {} }],
      },
    ...
    return "Sorry, I encountered an error verifying the tax data. Please check your connection and try again.";
  }
};